...
class OpenAI_REST_Controller extends WP_REST_Controller {
    ...
    public function log_request_dalle_generation( array $response ) {
        // Nothing here.
    }

    public function log_request_gpt_completion( array $response ) {
        // Nothing for now.
	...
	public function request_dalle_generation( WP_REST_Request $request ) {
        ...
        $this->log_request_dalle_generation( $api_call );
	...
	public function request_gpt_completion( WP_REST_Request $request ) {
        ...
        $this->log_request_gpt_completion( $api_call );
		if ( is_wp_error( $api_call ) ) {
			return $api_call;
		}
		$result = json_decode( $api_call['body'] );
